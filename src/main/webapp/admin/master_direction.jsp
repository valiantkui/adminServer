...
<html>
	<head>
		...
		function deleteDirection(d_no){
			...
			if(isDelete == "yes"){
				//确认要删除，则请求删除
				window.location = "/adminServer/master/deleteDirectionByD_no?d_no="+d_no;
			}
		}
		</script>
	</head>
			...
			<div class="col-lg-12 col-sm-12 col-md-12">
				<h3>专业</h3>
					<c:forEach items="${requestScope.professionList }" var="profession">
					    <a href="/adminServer/master/masterDirectionList?p_no=${profession.p_no}&p_name=${profession.name}" class="btn btn-light">${profession.name }</a>
				    </c:forEach>
			</div>	
			<br/><br/><br/><br/><br/><br/>
			<div class="col-lg-12 col-sm-12 col-md-12">
				<a style=" float: right;" href="/adminServer/master/addMasterDirection"  class="btn btn-light">添加方向</a>
				<table class="table">
				    <thead>
				      <tr>
				        <th>方向编号</th>
				    ...
				    <c:forEach items="${requestScope.directionList }" var="direction">
				      <tr>
				        ...
				        <td> 
				        	<span><a href="/adminServer/master/updateMasterDirection?d_no=${direction.d_no}">修改</a></span>
